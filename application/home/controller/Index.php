...
class Index extends HomeBaseController
{
     ...
     public function get_url_all()
    {
        ...
        if(!empty($array[$module])){
            ...
            if(empty($if)) {$this->redirect('/home');}
    ...
    public function index()
    {
		...
		//新闻动态
		$article['info_new'] = $this->getArticleData(1);
		$article['info_zhi'] = $this->getArticleData(2);
        ...
        $this->assign('title','服务器托管_服务器租用_IP传输_DDOS防御-RAKsmart全球数据中心，提供一站式服务。');
	...
	public function getArticleData($c_id)
	{
		...
		$info_new = MArticle::getSelect($where_c,'a_id,c_id,a_title,a_con,a_url,a_updatetime',2,'a_id desc');
		if(!empty($info_new)){
